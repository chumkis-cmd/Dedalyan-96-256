...
(`--port N`, `--no-browser`). Две вкладки — шифрование и расшифровка;
показывает тот же конвейер, что и консольная версия, плюс карту конверта с
раскраской по сегментам и кнопку «подделать один бит», которая наглядно
демонстрирует, как MAC отвергает изменённый конверт.
